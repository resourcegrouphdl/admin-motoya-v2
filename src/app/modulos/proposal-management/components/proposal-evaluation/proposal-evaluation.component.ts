...
import { MatCardActions, MatCardContent, MatCardTitle, MatCardSubtitle, MatCardHeader, MatCard } from "@angular/material/card";
...
export class ProposalEvaluationComponent {

 propuestas$: Observable<Propuesta[]> | undefined;
  ...
  evaluarPropuesta(propuesta: Propuesta): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.procesarEvaluacion(propuesta.id, result.decision, result.comentarios);
      }
    });
  }

  private procesarEvaluacion(id: number, decision: 'aprobada' | 'rechazada', comentarios: string): void {
    this.propuestaService.evaluarPropuesta(id, decision, comentarios).subscribe({
      next: (success) => {
        if (success) {
          this.snackBar.open(
      ...
      error: (error) => {
        this.snackBar.open('Error al evaluar propuesta', 'Cerrar', { duration: 3000 });
      }
    });
  }
